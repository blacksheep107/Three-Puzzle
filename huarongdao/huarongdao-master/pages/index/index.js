...
autoGame(){
  var isOver=false;
  var map = new Map();//状态是否出现过
  var route=new Map();
  var operation_to_the_state=new Map();
  var dis=['w','a','s','d'];
  var changeId=[
  ...
  var des="123456780";//目标
  var node=this.data.numData;//当前状态
  var nownode="";
  for(var i=0;i<9;i++){
    if(node[i].isEmpty==false)  nownode+=node[i].num;
    else  nownode+='0';
  }
  route.set(nownode,-1);
  map.set(nownode,true);
  var queue = [];  
  queue.unshift(nownode);  
  while (queue.length != 0) {  
      ...
      for(var i=0;i<4;i++){
        if(changeId[pos][i] != -1){
          ...
          for(var j=0;j<9;j++){
            if(j==pos){
              sa+=item[changeId[pos][i]];
            }else if(j==changeId[pos][i]){
              sa+=c;
            }
            else sa+=item[j];
          }//交换
          if(sa==des){//到达
            isOver=true;
            route.set(des,item);
            operation_to_the_state.set(des,dis[i]);
            break;
          }
          if(!map.has(sa)){
            map.set(sa,true);
            queue.unshift(sa);
            route.set(sa,item);
            operation_to_the_state.set(sa,dis[i]);
         }
        }
      }
      if(isOver)  break;
  ...
  var op=[];//操作
  if(operation_to_the_state.get(des)==null)    return;
  op.push(operation_to_the_state.get(des));
  if(route.get(des)==null) return ;
  var find_route= route.get(des);
  var n=0;
  while(find_route!=null&&route.get(find_route)!=-1){
    n++;
    op.push(operation_to_the_state.get(find_route));
  ...
  for(var i=len;i>=0;i--){
    for(var j=0;j<9;j++){
      if(numData[j].isEmpty){
        pos=j;break;
      }
    }  
    var newpos;
    if(op[i]=='w') newpos=pos-3;
    else if(op[i]=='s') newpos=pos+3;
    else if(op[i]=='a') newpos=pos-1;
    else  newpos=pos+1;
    sleep(300);
    [numData[pos],numData[newpos]]=[numData[newpos],numData[pos]];//交换
    var step=len-i+1;
    this.setData({step, numData });
    this.gameOver();
  }
