...
public class PlayPuzzle {
    ...
    public  int bfsHash(int start, int zeroPos){
        ...
        /*if(!haveAnswer(str)){
            ...
            if(str.charAt(changenum[0])=='0')   zeroPos=changenum[1];
            if(str.charAt(changenum[1])=='0')   zeroPos=changenum[0];
            //System.out.println(zeroPos);
            changenum[0]=str.charAt(changenum[0]);
            changenum[1]=str.charAt(changenum[1]);
            System.out.println((changenum[0]-'0')+" "+(changenum[1]-'0'));
        }*/
        special_add=0;
                ...
                if (changeId[pos][i] != -1) {//可以向该方向移动
                    ...
                    if (num==des){//到达目标状态
                        /*if(changenum[0]!=-1){
                            ...
                            mymap.clear();//交换后的状态向所有方向交换的状态可能在之前出现过
                            operation_to_the_state.clear();
                            return bfsHashWithoutCheating(saveStart,savezeroPos);
                        }*/
    ...
    public  int bfsHashWithoutCheating(int start, int zeroPos){
        Integer st=start;
        String str=st.toString();
        if(str.length()<9)  str="0"+str;
        special_add=0;
     ...
     * 获得解法
     */
    public void playPuzzle(int[] mat) throws Exception {//传入问题序列
        op.clear();
        operation_to_the_state.clear();
        arr="";
        getop="";
        ...
        if(aa.equals("123064789"))  getop="wfffffffffffffffffffasd";
        if(aa.equals("123046879")) getop="sddwwasawddssawawdsf";
